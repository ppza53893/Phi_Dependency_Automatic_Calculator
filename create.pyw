...
# デッドロック回避
_CO_INITIALIZE = ctypes.windll.ole32.CoInitialize
_CO_INITIALIZE(None)
...
def cprint(string:str, colorcode=None, reset_color=False, **kwargs):
    """
    色を変える(System.Console)
    こだわりたすぎて作ったもの
    """
    ...
    valid = [
        ...
        'Red', 'Magenta', 'Yellow']
    editable = (isinstance(colorcode, int) and (0x00 <= colorcode and colorcode <= 0x0f)) or \
        (isinstance(colorcode, str) and colorcode in valid)
    if editable:
        Console.ForegroundColor = colorcode

    initial = '>>> ' if 'initial' not in kwargs else kwargs['initial']
    if 'end' in kwargs:
        Console.Write(initial + string + kwargs['end'])
    else:
    ...
    if reset_color:
        ...
        call_cmd('color')
        #Console.ForegroundColor = 0x0f #動かない
ecprint = functools.partial(cprint, colorcode=0x04, reset_color=True) # pylint: disable=invalid-name
...
def call_cmd(command:str, **kwargs):
    ...
    if isinstance(command, (str, list, tuple)):
        ...
        ecprint("[ERROR] 有効なコマンドではありません : " + str(command))


call_cmd("cls") # or Console.Clear()
...
# はじめの画面
cprint('#'*CMD_TERMINAL_LENGTH, 0x0a, initial='')
cprint("Phi Dependency Automatic Calculator v2.00 (Update : 2021/01/24, Created by OHASHI)", initial='')
cprint("使用方法は「How to use.txt」を見てください。", 0x0e)
cprint("フォルダ参照を使用する場合、初回は少しロードが遅くなります")
cprint("ブラウズ画面やプログレスバーが表示されないときは何かのキーを押してください。")
cprint('#'*CMD_TERMINAL_LENGTH+'\n', 0x0a, initial='')
...
def browse():
    ...
    if FOR_VEE:
        ...
        if INPUT_PATH == "":
            cprint(">>> キャンセルされました。", reset_color=True)
            sys.exit(0)
        DIR_SELECTED = True
    ...
    for _path in iglob(escape_chr(INPUT_PATH)+'/*'):
        ...
        try:
            ...
            try:
                ...
                ecprint(f'[ERROR] ファイルを読み込めません({os.path.split(_path)[1]})。ファイル名を確認してください。')
                continue
        nums[_path] = num

    if len(nums) == 0:
        ecprint('[ERROR] 有効なファイルがありませんでした。')
        if FOR_VEE:
            sleep(3)
            sys.exit(1)
        cprint('ほかのデータを参照しますか? (y/n):', 0x0a, end='')
        return
...
if __name__=='__main__':
    ...
    while True:
        ...
        cprint(f"{chr(key)}", initial='', end='')
        if key==3 or chr(key).lower()=='n':
            cprint('', reset_color=True, initial='')
            sys.exit(0)
            ...
            if key == 0x1B:
                cprint('0', initial='', end='')
            cprint('無効なキーです。「y」か「n」を押してください:', 0x0a, initial='\n>>> ', end='')
